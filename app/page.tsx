...
export default function Home() {
  ...
  const handleMicClick = (taskId?: number) => {
    if (taskId) {
                      ...
                      <div className="p-4">
                        <div className="flex items-start gap-4">
                          <button
                            ...
                            className={`w-3 h-3 mt-1.5 rounded-full transition-colors ${getStatusColor(todo.status)}`}
                          ...
                          <button
                            onClick={() => handleMicClick(todo.id)}
                            className="opacity-0 group-hover:opacity-100 text-white hover:text-purple-400 transition-all p-2 hover:bg-purple-500/10 rounded-lg"
                          >
                  ...
                  <div key={project.id} className="bg-white/5 rounded-lg overflow-hidden">
                    <div className="p-4 border-b border-white/10">
                      <div className="flex items-center justify-between">
                        ...
                        <button
                          onClick={() => handleMicClick(project.id)}
                          className="text-white/60 hover:text-purple-400 transition-all p-2 hover:bg-purple-500/10 rounded-lg"
                        >
                          <Mic className="w-4 h-4" />
                        </button>
                      </div>
                    ...
                    <div className="divide-y divide-white/5">
                      {project.tasks.map((task) => (
                        <div key={task.id} className="p-3 hover:bg-white/5 group transition-colors">
                          <div className="flex items-center gap-3">
                            ...
                            <button
                              onClick={() => handleMicClick(task.id)}
                              className="opacity-0 group-hover:opacity-100 text-white/60 hover:text-purple-400 transition-all"
                            >
                              <Mic className="w-4 h-4" />
                            </button>
                          </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-end sm:items-center justify-center p-4 z-20">
          <div className="bg-zinc-900/90 w-full max-w-2xl rounded-t-lg sm:rounded-lg shadow-xl flex flex-col max-h-[85vh] backdrop-blur-sm">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {error && (
                <div className="mb-4 p-4 bg-red-500/10 text-red-500 flex items-start gap-2 rounded-lg border border-red-500/20">
              ...
              <div className="space-y-4">
                ...
                {getRecentMessages().map((message) => (
                  ...
                  >
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${
                      message.isUser ? 'bg-white/10' : 'bg-purple-500/20'
                    }`}>
                      {message.isUser ? (
                    ...
                    <div className={`flex-1 space-y-1 ${message.isUser ? 'text-left' : 'text-right'}`}>
                      <p className={`inline-block rounded-lg max-w-[80%] p-3 ${
                        message.isUser 
                          ? 'bg-white/5' 
                ...
                {transcript && (
                  <div className="flex gap-3">
                    <div className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 bg-white/10 animate-pulse ring-2 ring-purple-500/50">
                      <svg className="w-4 h-4 text-white/80" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                      </svg>
                    </div>
                    <div className="flex-1">
                      <p className="text-white/90 bg-white/5 p-3 inline-block rounded-lg max-w-[80%] border border-purple-500/20 animate-pulse">
                        {transcript}
                      </p>
                    </div>
                  </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className={`flex items-center justify-center gap-2 px-4 py-2 rounded-lg transition-all duration-200 ${
                    isListening
                      ? 'bg-purple-600 hover:bg-purple-700 ring-2 ring-purple-500/50'
